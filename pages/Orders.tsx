...
const Orders: React.FC = () => {
  ...
  // URL statusni tahlil qilish (default: ALL)
  ...
  const fetchData = async () => {
    try {
      ...
      setCurrentPage(1); // URL status o'zgarganda birinchi sahifaga qaytish
    } catch (err: any) {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      showNotification("Status muvaffaqiyatli yangilandi", "success");
      
      setOrders(prev => prev.map(o => o.id === id ? { 
        ...o, 
        status: nextStatus, 
        statusDescription: getStatusLabel(nextStatus)
      } : o));
    } catch (err: any) {
  ...
  const getStatusStyle = (code: string) => {
    switch (code) {
      ...
      default: return 'bg-slate-100 text-slate-700 dark:bg-slate-500/20 border border-slate-200';
    }
  };
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-4 sm:px-0">
      ...
      <div className="bg-indigo-600 rounded-[2.5rem] p-8 sm:p-12 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-indigo-100 font-bold uppercase text-[10px] tracking-[0.4em] mt-2 opacity-80">
            {getStatusLabel(filterStatus)} ro'yxati
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 px-2">
        <div className="flex items-center gap-3">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Sahifa hajmi:</span>
          <div className="flex bg-white dark:bg-slate-900 rounded-xl p-1 shadow-sm border border-slate-200 dark:border-slate-800">
            {[5, 10, 20].map(size => (
              <button
                ...
                className={`px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${pageSize === size ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-indigo-600'}`}
              >
                {size}
              </button>
            ))}
        ...
        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-slate-900 px-4 py-2 rounded-full border border-slate-200 dark:border-slate-800">
        ...
        <div className="space-y-6">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800">
                  <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Vaqt</th>
                  <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Mijoz</th>
                  <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Telefon</th>
                  <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest">Summa</th>
                  <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Buyurtma</th>
                  <th className="px-8 py-8 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Status / Amal</th>
                </tr>
              </thead>
                ...
                {paginatedOrders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-indigo-500/5 transition-all group">
                      ...
                      <td className="px-8 py-6">
                         <span className="text-[11px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-xl">{order.phone}</span>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6 text-center">
                        <button 
                          onClick={() => setSelectedOrderItems({items: order.items, orderId: order.id})}
                          className="px-5 py-2.5 rounded-2xl bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all shadow-sm"
                        >
                        ...
                        <div className="flex justify-end items-center gap-3">
                           {updatingId === order.id ? (
                             ...
                             <div className="flex gap-2">
                                <button 
                                  onClick={() => setConfirmModal({id: order.id, nextStatus: 'SOLD', label: 'Sotib olingan'})}
                                  className="px-4 py-2 rounded-xl bg-emerald-500 text-white text-[9px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-md shadow-emerald-500/20"
                                >
                                  ✅ Sotildi
                                </button>
                                <button 
                                  onClick={() => setConfirmModal({id: order.id, nextStatus: 'CANCELED', label: 'Bekor qilingan'})}
                                  className="px-4 py-2 rounded-xl bg-red-500 text-white text-[9px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-md shadow-red-500/20"
                                >
                                  ❌ Bekor
                                </button>
                             </div>
              ...
              return (
                <div key={order.id} className="glass p-6 rounded-[2.5rem] shadow-xl border border-white/20 relative overflow-hidden">
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className={`${getStatusStyle(order.status)} px-3 py-1.5 rounded-xl text-[8px] font-black uppercase`}>
                       {getStatusLabel(order.status)}
                    </div>
                  </div>
                  <div className="flex justify-between items-center mb-6 px-1">
                    <p className="text-sm font-black text-indigo-600">{total.toLocaleString()} {currency}</p>
                    <p className="text-xs font-bold text-slate-400">{order.phone}</p>
                  </div>
                  <div className="flex gap-3">
                    <button 
                      onClick={() => setSelectedOrderItems({items: order.items, orderId: order.id})}
                      className="flex-1 py-4 rounded-2xl bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest shadow-lg"
                    >
                      Ko'rish
                    </button>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 mt-8">
              <button disabled={currentPage === 1} onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} className="w-10 h-10 glass rounded-xl flex items-center justify-center disabled:opacity-30 font-black">&larr;</button>
              <div className="flex gap-1">
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => {
                  // Only show 1, last, and around current
                  if (page === 1 || page === totalPages || (page >= currentPage - 1 && page <= currentPage + 1)) {
                    return (
                      <button key={page} onClick={() => setCurrentPage(page)} className={`w-10 h-10 rounded-xl text-[10px] font-black transition-all ${currentPage === page ? 'bg-indigo-600 text-white' : 'glass'}`}>{page}</button>
                    );
              ...
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} className="w-10 h-10 glass rounded-xl flex items-center justify-center disabled:opacity-30 font-black">&rarr;</button>
            </div>
          )}
          ...
          {orders.length === 0 && !loading && (
            <div className="text-center py-40 glass rounded-[3rem]">
               ...
               <p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em]">Ushbu kategoriyada buyurtmalar mavjud emas</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="glass w-full max-w-xl rounded-[3rem] shadow-2xl border border-white/20 overflow-hidden animate-in zoom-in duration-500">
            ...
            <div className="p-8 sm:p-12 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white/50 dark:bg-slate-900/50">
              ...
              <button onClick={() => setSelectedOrderItems(null)} className="w-12 h-12 glass rounded-2xl text-2xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all">&times;</button>
            </div>
                  ...
                  <div key={idx} className="flex justify-between items-center p-5 rounded-3xl bg-indigo-500/5 border border-indigo-500/10">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center">
                        {p?.imgUrl ? <img src={p.imgUrl} className="w-full h-full object-cover" /> : <span>🧼</span>}
                      </div>
                      <div>
                        <span className="font-black text-slate-900 dark:text-white text-sm uppercase block">{p?.nameUz || 'Noma\'lum'}</span>
                        <span className="text-[9px] font-bold text-slate-400 uppercase">{p?.price.toLocaleString()} {p?.currency}</span>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-black text-indigo-600">{item.quantity} ta</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="p-8 sm:p-12 bg-slate-50 dark:bg-slate-900/50 flex flex-col gap-6">
              <div className="flex justify-between items-end border-b-2 border-dashed border-slate-200 dark:border-slate-800 pb-4">
                 <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Jami hisob:</span>
                 <div className="text-right">
                    ...
                    <p className="text-[10px] font-black text-indigo-600 uppercase mt-1">{calculateOrderTotal(selectedOrderItems.items).currency}</p>
                 </div>
              </div>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-200">
          <div className="glass rounded-[3rem] p-10 sm:p-12 max-w-sm w-full border border-white/20 text-center animate-in zoom-in duration-300">
            ...
            <div className="bg-amber-500/10 border border-amber-500/20 p-5 rounded-3xl mb-10 text-left">
              <p className="text-[10px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-widest mb-2">Eslatma:</p>
              <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400 leading-relaxed italic">
                ...
                Agar <span className="text-indigo-600 font-black">"Sotib olingan"</span> qilinsa, keyin bekor qilingan qilolmaydi, 
                agar <span className="text-red-600 font-black">"Bekor qilingan"</span> qilinsa, sotib olingan qilolmaydi.
              </p>
            </div>

            <div className="flex gap-4">
              <button onClick={() => setConfirmModal(null)} className="flex-1 py-4 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-500 font-black text-[10px] uppercase tracking-widest">Bekor</button>
              <button onClick={handleUpdateStatus} className={`flex-1 py-4 rounded-2xl text-white font-black text-[10px] uppercase tracking-widest shadow-lg ${confirmModal.nextStatus === 'SOLD' ? 'bg-emerald-500' : 'bg-red-500'}`}>Ha, Tasdiqlayman</button>
            </div>
          </div>
        </div>
      )}
